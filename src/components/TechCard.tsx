...
export const TechCard = ({ tech, index }: TechCardProps) => {
  ...
  const iconFromConfig = Icons[tech.icono as keyof typeof Icons];
  const Icon = (typeof iconFromConfig === 'function'
    ? iconFromConfig
    : Box) as LucideIcon;
    ...
    >
      ...
      {showTooltip && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: 10 }}
          className="absolute z-50 left-1/2 -translate-x-[58%] top-full mt-3 w-64 p-3 text-white text-xs rounded-lg shadow-xl border border-gray-300"
